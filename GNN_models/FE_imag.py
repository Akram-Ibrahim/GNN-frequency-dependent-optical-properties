...
#### Multi-Fidelity (Fidelity-Embedding) AvgNorm GNN
...
with open('./dataset/mbj_avg_data.json', 'r') as f:
    data = json.load(f)
dff_mbj = pd.DataFrame(data)


with open('./dataset/opt_avg_data.json', 'r') as f:
    data = json.load(f)
dff_opt = pd.DataFrame(data)   
##################     


# Add a new column 'fidelity' to each DataFrame with a list
dff_opt['fidelity'] = [[0] for _ in range(len(dff_opt))]
dff_mbj['fidelity'] = [[1] for _ in range(len(dff_mbj))]


# convert cif to pymatgen structures
def cif2pystructure(row):
    py_struc = Structure.from_str(row['cif'], fmt='cif')
    # Add the state to the py_structure
    py_struc.state = row['fidelity']
    return py_struc

# Apply the function to create the 'pystructure' column
dff_mbj['pystructure'] = dff_mbj.apply(cif2pystructure, axis=1)
# Apply the function to create the 'pystructure' column
dff_opt['pystructure'] = dff_opt.apply(cif2pystructure, axis=1)
######################
...
# Apply the function to dff_mbj and dff_opt
dff_mbj = filter_by_number_density(dff_mbj)
dff_opt = filter_by_number_density(dff_opt)
######################
...
dff_mbj['band_gap_index'] = dff_mbj.apply(find_closest_index, axis=1)
dff_opt['band_gap_index'] = dff_opt.apply(find_closest_index, axis=1)
...
dff_mbj = dff_mbj.apply(set_values_to_zero, axis=1)
dff_opt = dff_opt.apply(set_values_to_zero, axis=1)
##################
...
selected_columns = ['pystructure', 'band_gap', 'band_gap_index', 'energy_range', 
                    'imag_avg_interp', 'fidelity']

# Create the sub-DataFrame from the main DataFrame
df_mbj = dff_mbj[selected_columns]
df_opt = dff_opt[selected_columns]
##################

# Count non-None values in each column
non_none_count_mbj = df_mbj.count()
non_none_count_opt = df_opt.count()

print('mbj count:', non_none_count_mbj)
print('opt count:', non_none_count_opt)
##################
...
df_mbj['imag_normalized'] = df_mbj['imag_avg_interp'].apply(normalize_target)
df_mbj['imag_norm'] = df_mbj['imag_avg_interp'].apply(target_norm)

df_opt['imag_normalized'] = df_opt['imag_avg_interp'].apply(normalize_target)
df_opt['imag_norm'] = df_opt['imag_avg_interp'].apply(target_norm)
##################
#----------------#
...
# Only train/test split MBJ data
df_train_mbj, df_test, df_left = train_validate_test_split(df_mbj)

# Use all of OPT data for training
df_train = pd.concat([df_train_mbj, df_opt], ignore_index=True)
...
norms_train = np.expand_dims(np.array(df_train['imag_norm'].tolist()), axis = -1)
normalized_vector_train =  np.array(df_train['imag_normalized'].tolist())
targets_train = np.concatenate((norms_train, normalized_vector_train), axis=1)
###
###
structures_test = df_test['pystructure'].tolist()
norms_test =  np.expand_dims(np.array(df_test['imag_norm'].tolist()), axis = -1)
normalized_vector_test =  np.array(df_test['imag_normalized'].tolist())     
...
pointwise_spectrum_shape = df_mbj['imag_avg_interp'].iloc[0].shape[0]
print('pointwise_spectrum_shape = ', pointwise_spectrum_shape)
##################
# #### Get the energy grid
en_grid = df_mbj['energy_range'].tolist()
...
def build_keras_model(n_feat_atom, nfeat_bond, n_feat_global, activ, dropout_rate, energy_grid_size, 
    ...
    #x3_ = x3
    x3 = Input(shape=(None,), dtype='int32', name="fidelity_input") 
    x3_ = Embedding(input_dim=2, output_dim=n_feat_global, name="fidelity_embedding")(x3)
    ...
    # Identity layer
    x3_ = Dense(n4, activation= activ, name="global_ff_layer_1")(x3_)

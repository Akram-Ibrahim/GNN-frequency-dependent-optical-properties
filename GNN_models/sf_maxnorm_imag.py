...
# nomalize as maxnorm
def normalize_target(vector): # spectrum normalized as maxnorm
...
df['imag_normalized'] = df['imag_avg_interp'].apply(normalize_target)
df['imag_norm'] = df['imag_avg_interp'].apply(target_norm)
...
def build_keras_model(n_feat_atom, nfeat_bond, n_feat_global, activ, dropout_rate, energy_grid_size, 
    ...
    def scale_y2_pred(x):
        max_value = kb.max(x, axis=-1, keepdims=True)
        scaled_y2_pred = x - 0.5 * max_value
        return scaled_y2_pred

    # Scale y2_pred such that its 1/2 max is at zero
    y2_pred = Lambda(scale_y2_pred, output_shape=(1, None, energy_grid_size), name='half-max_scaling')(y2_pred)


    # Add the softmax layer 
    y2_pred = Dense(energy_grid_size, activation='sigmoid', name="logits_normalized")(y2_pred)
...
def custom_loss_wrapper(energy_grid_size, energy_grid_delta, loss_weights):
    def custom_loss(y_true, y_pred):
        ...
        ## ----------------- ##
        component1 = 0.002 * kb.mean(kb.abs((y1_pred - y1_true)), axis=-1) # average value loss (norm loss)
        ## ----------------- ##    
        component2 = 1 * kb.mean(kb.abs(y2_pred - y2_true), axis=-1) # Spectrum point-wise loss 
...
def metric_1_wrapper(energy_grid_size, energy_grid_delta, loss_weights):
    def component_1(y_true, y_pred):
        ...
        ## ----------------- ##
        component1 = 0.002 * kb.mean(kb.abs((y1_pred - y1_true)), axis=-1) # norm loss
...
def metric_2_wrapper(energy_grid_size, energy_grid_delta, loss_weights):
    def component_2(y_true, y_pred):
        ...
        ## ----------------- ##
        component2 = 1 * kb.mean(kb.abs(y2_pred - y2_true), axis=-1) # normalized spectrum loss 
